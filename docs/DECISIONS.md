...
## 2026-08-05 — Blog frontmatter `affiliate` field + build-time schema guard
- Status: accepted
- Decision: `content/blog/*.md` frontmatter accepts `affiliate: { program, items[], subId? }`. `resolveAffiliate()` in `app/lib/affiliate.js` maps item ids to the Coupang catalog and returns `AffiliateBox` props; unknown program/item/empty list throws at build time. Each link gets `?subId=<slug>` (overridable) and `AffiliateBox` sends `affiliate_placement` to GA4. `app/lib/blog.js` validates every post against an allowed-key list and the two valid `status` values, failing the build on a typo (absence is not checked — the upcoming stubs have no ogImage/pillar/faq).
- Rationale: Blog posts are plain markdown rendered by react-markdown without `rehype-raw`, so a component written into the body compiles and then silently disappears. Reusing the FAQ pattern (frontmatter → template) keeps one mechanism for both. The disclosure text is a compliance requirement, so it lives in one module rather than being copied per page. Silent failures are the real risk here: a missing affiliate box or a `status: Published` typo is invisible until someone reads the deployed HTML, and a failed build only blocks the deploy step while the previous deploy keeps serving.

## 2026-08-07 — Place image scan accepts `.webp` only
- Status: accepted
- Decision: `scripts/build_places.mjs` scans `public/images/places/{slug}/` for `.webp` files only. Source formats (`.jpg`, `.jpeg`, `.png`) are ignored rather than turned into Cloudinary public ids.
- Rationale: The scan strips the extension to form a public id, but only `.webp` is ever uploaded — `optimize-images` generates it and `upload-cloudinary` pushes it. Accepting source formats minted ids for objects that were never uploaded: four stray originals in `cafe-pepper/` produced four live 404s that went unnoticed from 07-24 because the eval that checks image integrity had not run in four months. That directory is generated output (CLAUDE.md Forbidden), so a non-webp file in it is a leftover, not a deliverable. `data/places.json` drops from 83 to 79 image references.

## 2026-08-07 — Judgment records as a data model + harness conformance checks
- Status: accepted
- Decision: Two new artifact types. (1) `prompts/judge-{seo-eeat,accuracy-safety}.{md,rubric.json}` — the dual-judge publishing gate as prompts plus machine-readable rubrics; the rubric JSON is the sole definition of points, read by the prompt renderer, the response schema, and the tally. (2) `eval/judgments/<slug>.json` — one record per judged post carrying `content_sha256`, `rubric_version`, model, effort, per-criterion points, findings, and a `provenance` enum whose only permitted value is `"runner"`. `scripts/check-harness.mjs` verifies both the rule documents (cited paths, cited npm scripts, baseline freshness, eval/README accuracy, forbidden files) and the judgment records (present, hash-current, rubric-current). No new npm dependencies — `@anthropic-ai/sdk` and `gray-matter` were already present.
- Rationale: The "two judges, both 9.5/10" rule had existed since July as prose in HANDOFF.md with no prompt, rubric, runner, or recorded score — a gate nobody could re-run. Splitting it into a paid non-deterministic judge track and a free deterministic evidence track lets CI verify that judging happened without an API key, and makes editing a post after it was judged break the hash. `provenance` is an enum of one so that a hand-written score is unrepresentable rather than merely discouraged. The model returns per-criterion integers and never a total: asked for "a score out of 10" it picks the number first and justifies backwards. The response schema is built by hand instead of via `jsonSchemaOutputFormat()`, which rewrites `enum` into a `description` string and would demote the point ranges from constraint to suggestion.
