...
## ▶ 로마자 철거 + 법령명 정정 — ✅ 커밋됨 `98dfc2f` (2026-09-07) · 🔴 push 안 함

**커밋은 됐고 push는 안 했다 → 라이브는 아직 옛 상태다**(push = 자동배포라 사람이 누른다). 8/27의 *"운영자 입력 받고 한번에"* 전제가 **2026-09-05에 깨져서** 분리했다 — 빵집 일화 세부가 Ki 기억에 없어 회수 불가로 확정됐고(→ memory `feedback_experience_details_unrecoverable`), 그 사이 라이브에 틀린 법령 인용이 계속 걸려 있었다. 운영자 결정: *"떼서 해줘"*.

🔑 **eval/judgments 는 일부러 커밋에서 뺐다.** 판정 파일이 글의 `content_sha256` 을 기록하는데 **9/07에 고친 3편(celiac·phrases·labels)이 어긋나고 나머지 3편은 일치**한다. 지금 재채점하면 통과 상태인 `celiac`(9.5/9.5)이 재롤에서 떨어질 위험이 있다 → **experience 작업이 끝날 때 6편을 함께 돌린다.**

**9/07에 추가로 처리한 것 2건:**
- **알레르기 항목 수 19/21/22 확정.** 8/25부터 "미확인"으로 남아 있던 대목. 법제처에서 **별표2 원문 PDF를 받아** 셌다 — 조문 그대로 **19**, 조개류를 굴·전복·홍합으로 쪼개면 **21**, 조개류를 항목으로 두고 셋을 더하면 **22**. 기존 문장은 21과 22를 **모두 "쪼갠 것"으로 설명해 22를 설명하지 못했다.**
- **확인일 → 개정 표기.** `text in force as of August 2026` → **「총리령 제2004호, 2024-12-30 개정, 2026-01-01 시행」** 3편. 법제처 본문에서 직접 읽었다. 판사의 authoritativeness minor 3편이 이걸로 닫힌다.

🔴 **미해결로 남긴 것:** `hidden` 당면 **혼합 전분 주장이 무출처**다. 검색 2회·제품 페이지 2곳을 봤으나 **소매가 원재료명을 이미지로만 싣는다.** 근거 없이 두거나 문장을 약화시키는 건 운영자 판단이라 손대지 않았다.

**① 어감 교체(운영자 답):** `밀가루 조금이라도 섞나요?` → **`들어가나요?`**. 글의 다른 문장이 전부 `들어가나요` 형태였고 이 줄만 튀고 있었다.

**② 로마자 전면 철거(운영자 답 — 8-25 미확인 항목 해소):** 5편 43건. HANDOFF엔 `labels`·`hidden-gluten` 2편으로 적혀 있었으나 **`celiac`·`butter-tteok`에도 있었다**(`mil-tteok`·`baekseolgi`·`ppeongttwigi`). 기준·잔존 판단은 memory `feedback_show_dont_pronounce` — 요약하면 **발음 표기는 삭제하고 그 자리에 뜻을 넣되, 영어권에서 이름으로 굳은 단어(gochujang·tteok·dangmyeon)와 브랜드명(Pocachip)은 남긴다.** 판정의 1차 자료는 이미 정리된 `korean-gluten-free-phrases`를 열어서 확인했다.

**③ 법령명 정정:** HANDOFF의 4곳 + **전수 확인으로 2곳 더**(`celiac:224` 목록 링크, `butter-tteok:59` 혼입 주의문구 의무) = 6곳. 전부 **「식품등의 표시·광고에 관한 법률 시행규칙 별표 2」**(소비자 안전을 위한 표시사항, 제5조제1항)로. 고시(표시기준)는 **표시 방법** 근거로만 남겼다. 변경 이력 줄이 *"added the MFDS labeling-standard citation"*이라고 틀린 인용을 자랑하던 것도 4편 정정.

🔑 **8-25가 제안한 URL은 200이었지만 1,269바이트 iframe 껍데기였다.** 안쪽까지 열어 별표 2 직결 URL을 찾았다 — `<title>` = **"별표·서식 > 소비자 안전을 위한 표시사항(제5조제1항 관련)"**, `lsiSeq`가 **버전 고정**(시행 2026-01-01)이라 판사가 요구하는 "어느 버전을 읽었는지"를 URL이 답한다. `labels`의 약한 식품안전나라 링크 3곳도 교체.
`https://www.law.go.kr/LSW/lsBylInfoPLinkR.do?lsiSeq=267855&bylNo=0002&bylBrNo=00&bylCls=BE&bylClsCd=BE`
⚠️ **검증한 건 제목까지다** — 법제처는 본문이 JS 렌더라 19개 목록을 이 URL에서 못 긁는다. 목록 내용은 8-25 PDF 직독분.

### 재채점 결과 (6편, 각 75초) — 2 PASS / 4 FAIL

| 글 | 직전 | 지금 | |
|---|---|---|---|
| `snacks` | 10 / 9.5 (8-13) | **10 / 9.5** | ✅ |
| `celiac` | 10 / 9.5 (8-25) | **9.5 / 9.5** | ✅ |
| `phrases` | 9.5 / 9.5 (8-25) | 9.5 / **9** | ❌ |
| `labels` | 9.5 / 9.5 (8-25) | 9.5 / **9** | ❌ |
| `hidden-gluten` | 10 / 9.5 (8-15) | 9.5 / **9** | ❌ |
| `butter-tteok` | 9.5 / 9.5 (8-14) | **8.5** / 9.5 | ❌ |

⚠️ **하락을 이번 수정 탓으로 읽지 말 것.** `phrases`·`labels`는 8-25에도 major 1건을 안은 채 9.5로 통과했고, `butter-tteok`·`hidden-gluten`은 **2주 만의 재롤**이다(판사 운영지식 ⑩ — 마진 0은 다음 재롤에서 떨어진다).

🔴 **FAIL 4편의 blocking은 전부 같은 하나 = `experience` major.** *"상호도 동네도 월도 없어 겪은 일과 잘 조사한 글을 구분할 수 없다."* **운영자 입력 없이는 4편 다 PASS 불가** → 남은 minor를 지금 고치는 건 재채점 낭비. 입력 오면 한 번에.

**▶ 운영자에게 받을 것 (이거 하나로 4편이 열린다):**
1. **빵집 "10% 밀" 일화 — 상호 · 동네 · 월** (`phrases`·`labels` 공통, 세 편째 같은 자리)
2. **실패담 1건 — 어떤 문장이 어떻게 빗나갔는지**
3. `butter-tteok`: 6,000원 영수증의 **카페 이름·시점** + 집에서 만들 때 **밀가루 무게 · 몰드 크기 · 2봉지 가격**
4. `hidden-gluten`: 온라인 빵 **재라벨 사건의 판매처·월**

**▶ 판사가 이번 변경에 건 반론 2건 (운영자 판단 필요):**
- 🔴 **로마자 제거 → `expertise` minor 3편.** *"복사 버튼 정책은 syndication에서 무너진다 — Google FAQ rich result와 **AI 답변엔 복사 버튼이 없다**"*, *"밀떡은 핵심 용어인데 발음·타이핑·**검색**을 못 한다."* 운영자 방침 우선이라 되돌리지 않았다. 다만 **AI 인용 노출이 현 전략 축**이라 문장(≠용어)만 복사 버튼으로 가고 **용어는 검색 가능성**을 따로 볼 여지가 있다.
- **`authoritativeness` minor 3편:** *"'in force as of'는 확인일이지 **개정일**이 아니다. law.go.kr이 개정일을 노출한다."* → 개정일 stamp로 교체하면 닫힌다(내가 처리 가능).

**▶ 이번에 판사가 새로 잡은 것:**
- ✅ **처리함** — `labels:43` 출처 문장이 아직 두 법령을 뭉뚱그렸다(판사가 내 정정이 맞다고 확인하면서 잔여를 지목). 시행규칙 별표 2(목록·20mg/kg·혼입 의무) / 고시(표시 방법)로 분리.
- 🔴 **미처리 실오류** — `labels`: **"조개류를 굴·전복·홍합으로 쪼개면 19→21이지 22가 아니다."** 8-25에 "19 vs 22는 조개류 분할로 보이나 미확인"이라 적어둔 그 대목. 산수가 안 맞으니 **별표 2 원문으로 재확인 필요**.
- `hidden-gluten` **major 1건 추가**: 당면 조성 주장이 무출처(먹는 방향을 가리키는 유일한 무출처 주장). 어묵·카레루·소시지 절도 같은 계열.

**▶ 재개 절차:** 운영자 입력 반영 → `npm run judge -- <slug>` (6편, 편당 75초) → `npm run build && npm run validate:places` → 커밋·push(자동배포). 판사 실행 중 글 수정 금지(해시 기록).
...
- **마지막 업데이트:** 2026-09-07 08:41
- **작업자:** Claude Code
- **마지막 커밋:** `98dfc2f` content: 법령 인용 정정·로마자 철거·알레르기 항목 수 교정
- **브랜치:** main
