...
**보강 내용**: Step 2를 "밀 한 글자 사냥"에서 **"그 줄을 읽는 법"**으로 바꿈 — 표시대상 **19개 전체 해독표**(한글/영어) + 규정 자체의 예시(`달걀, 우유, 새우, 이산화황, 조개류(굴) 함유`) + **함유 / 혼입 가능 / 같은 제조시설** 3단 구분표. 🔑 목록을 실으니 **"밀만 표시대상, 보리·호밀은 아니다"가 주장이 아니라 독자가 눈으로 세는 사실**이 됐다. 🔑 그리고 원문에서 **혼입 표시는 이미 원재료로 declared된 경우 붙이지 않는다**는 단서를 확인 — 즉 혼입 경고가 없는 이유가 정반대 둘("경고할 게 없다" / "이미 위에 적혀 있다")이라 **함유 줄을 먼저 읽어야 한다.** FAQ 2개 추가(표시줄 단어 해독 / 글루텐프리 확인법).

🎉 **HANDOFF 미해결 항목 해소 — 무글루텐 20ppm을 1차 확인했다.** 법제처 PDF를 받아 직접 읽음: **「식품등의 표시·광고에 관한 법률 시행규칙」 별표 2 Ⅰ-3** — *"밀, 호밀, 보리, 귀리 또는 이들의 교배종을 원재료로 사용하지 않고 총 글루텐 함량이 1킬로그램당 20밀리그램 이하인 식품등"* (또는 글루텐 제거 원재료 사용 + 동일 상한)에 **"무 글루텐" 표시를 할 수 있다.** 🔑 **"할 수 있다"=임의 표시**이고 제3자 인증(GFCO 등)은 여전히 없다 → *숫자엔 법적 근거가 있고 검증은 제조사 몫*이라는 정확한 구도로 쓸 수 있게 됐다. 그리고 **우리가 읽은 조항에 `저글루텐`의 정의는 없다** — 무 글루텐만 정의·상한이 있고 저글루텐은 대응 조항이 없어, "저글루텐은 주장이 아니라 경고"라고 쓸 근거가 생겼다.

🔴 **발행 6편 중 5편이 같은 법령을 잘못 인용하고 있다.** 알레르기 표시대상 **목록의 근거는 「식품등의 표시기준」(식약처 고시)이 아니라 「식품등의 표시·광고에 관한 법률 시행규칙」 별표 2**(총리령, 제5조제1항 관련)다. 별표 제목("소비자 안전을 위한 표시사항")은 우리가 맞게 썼는데 **상위 법령명을 틀렸다.** 고시는 표시 *방법*을 정한다. `labels`는 이번에 정정(본문 2곳 + FAQ 1곳). **남은 4편** = `korean-gluten-free-phrases:123` · `celiac-travel-korea-guide:206` · `hidden-gluten-korean-food:48` · `korean-convenience-store-gf-snacks:109`. ▶ **정정하면 재채점 4편** — 운영자 판단 대기.

**▶ 같이 처리할 것 (판사 지적):** `labels`의 별표 2 링크가 지금 **식품안전나라 게시판**이라 중심 주장(19개 목록)을 지탱하기엔 약하다. **법령 원문 URL이 살아 있음을 확인했다(200)**: `https://www.law.go.kr/법령/식품등의표시·광고에관한법률시행규칙` — 위 4편 정정과 한 번에 묶는 게 효율적이다.

### 📌 라벨 읽기 콘텐츠 요청 (2026-08-25, 운영자) — 마이노멀 링크는 계기였다
...
- 🔴 **운영자 정정: 이 제품을 소개하려던 게 아니다.** 마이노멀은 **편의점에서 팔지 않는다**(네이버쇼핑에서 찾은 것). 링크는 예시였고 실제 요청은 이것이다 — *"편의점에서 고를 때 뒤에 있는 **대두·우유**가 무슨 뜻인지, **밀 함유**된 걸 어떻게 피하는지, 어떻게 글루텐프리인지 확인하는지"*를 알려주고 싶다. **제품 후보가 아니라 콘텐츠 방향이었다.** 🔑 던져진 링크를 "등록할 대상"으로 읽는 것이 첫 오독이었다 — 링크는 대상이 아니라 **예시**일 수 있다.
- ⚠️ 품목("과자·스낵")까지만 운영자 확인. `mynormal.shop` 상세는 **403**, 네이버는 **429**라 제품 특정은 여전히 안 됨. 콘텐츠 방향이 바뀌었으므로 **제품 특정은 더 이상 blocking이 아니다.**

**▶ 갭 진단 (실측):** 요청 내용은 발행글 두 편 사이에 끼어 있다.

| 요청 요소 | `reading-korean-food-labels` | `korean-convenience-store-gf-snacks` |
|---|---|---|
| 편의점 실전 맥락 | 약함 | ✅ |
| **알레르기 표시줄 단어 해독(대두·우유…)** | 🔴 **없음** — `대두`·`우유` 본문 등장 0건 | 🔴 없음 |
| 밀 함유 피하기 | ✅ Step 2~3 | 링크로 위임 |
| 글루텐프리 확인법 | ✅ Step 4 + 제조사 문의 | 링크로 위임 |

🔑 **진짜 빠진 건 하나다: 알레르기 표시줄에 줄줄이 적힌 단어가 각각 뭔지.** 지금 글은 *"밀을 찾아라"*(타겟 검색)인데 운영자가 말한 건 *"그 줄을 읽을 줄 알아라"*(판독 능력)다. 한국어를 모르면 그 줄 전체가 벽이고, 밀을 찾으려면 먼저 그 줄을 읽을 수 있어야 한다.

**▶ 보강이 새 글보다 나은 이유:** `labels` 글의 **Step 2가 정확히 그 자리**이고, 표시대상 전체 목록을 보여주면 이 글의 중심 논리(**밀만 표시대상, 보리·호밀은 아니다**)가 **눈으로 보이게** 된다 — 목록에 보리·호밀이 없는 걸 독자가 직접 확인한다. 같은 주제로 새 글을 쓰면 카니발라이제이션이고, 그건 AdSense 반려 사유(얇은 중복 페이지)를 재생산한다.

⚠️ **쓰기 전 1차 확인 필요**: 표시대상 품목 수가 자료마다 **19 vs 22**로 갈린다(조개류를 굴·전복·홍합으로 쪼개 세느냐 차이로 보이나 미확인). 기존 글이 이미 **고시 제2025-60호**를 인용하므로 같은 고시의 **별표 2 원문**을 봐야 한다. 숫자를 2차 출처에서 베끼지 말 것.
...
**구현**: `app/blog/[slug]/page.js`의 ReactMarkdown에 `components` 추가(`td`·`blockquote`). 복사 텍스트는 **조각 단위로 한글이 있는 것만** 모은다 — 인용문에는 영어 번역이 형제 노드로 있어서 통째로 복사하면 주방에 영어 섞인 문장을 내밀게 된다. 실측 확인: 라틴문자 혼입 0. 만들면서 부작용 2건을 잡았다 ①react-markdown v10이 넘기는 `node` prop이 DOM에 `node="[object Object]"`로 누출 ②**라벨 인쇄 문구**(`"…같은 시설에서 제조"`)에도 버튼이 붙음 — 그건 독자가 라벨에서 *찾을* 문구이지 보여줄 문장이 아니다. → 문장 종결(`[가-힣][다요죠][.?!]`) 조건으로 좁혀 해결. **이 컴포넌트는 전 블로그 글에 적용된다**(pillar·labels의 문의 문장에도 붙음 = 의도된 것).

🔴 **판사가 잡은 major — 전략 문구의 구조적 구멍:** **밀가루는 wheat *flour*를 가리키는데 양조간장은 밀(곡물)로 발효된다.** 그래서 주방이 간장 든 요리에 "밀가루는 안 들어가요"라고 **정직하게** 답해도 그 요리엔 밀이 있다. 우리 글 전체가 "밀가루 알러지" 프레이밍인데 그 한계를 말한 적이 없었다. → 3절을 "이 섹션이 선택이 아닌 이유"로 다시 씀 + 짧은 오프너는 **밀가루**(통하는 말), 스크린샷 완전판은 **밀**(정확한 말)로 갈라 그 차이를 명시.

⚠️ **같은 실수를 두 번 했다.** 1라운드에서 스크린샷 블록의 간장 서술이 틀려 고쳤는데, 4라운드에서 **본문에 밀가루≠밀을 확립해놓고 스크린샷 블록은 밀가루인 채로 뒀다**가 다시 major를 맞았다. 🔑 **본문 논리를 바꾸면 "독자가 그대로 복사해 갈 블록"을 같이 봤는지 확인할 것** — 그 블록은 본문과 따로 유통된다.

**5라운드 만에 PASS (9.5/9.5 · 9.5/9.5).** 라운드별로 잡힌 것: description이 'romanisation'을 광고하는데 본문은 없다고 선언(스니펫 모순) · "Say whichever you can pronounce"가 글 방침과 정면 충돌 · 조선간장을 밀 없음으로 단정(혼합간장이 옆에 있다) · 우리 관찰을 기사 목소리 일반론으로 서술. **pillar도 재채점 PASS(10/9.5 · 9.5/9.5).**

**▶ 남은 것:**
1. 🔴 **experience major가 안 풀린다 — 운영자만 답할 수 있다.** 빵집 "10% 밀" 일화에 **동네·월·빵집 종류**가 없다. 판사: *"a reader has nothing to weigh it against."* 여기에 더해 **구체적 실패담 하나**(어떤 문장이 어떻게 빗나갔는지)가 있으면 이 축이 닫힌다.
2. **내부 불일치 1건(보수적 방향이라 방치 중):** 본문은 근거 범위를 "Seoul, Gyeonggi and Cheonan"이라 하고 마지막 섹션은 "Seoul and Gyeonggi"라 한다. 고치면 재채점 1라운드가 붙어 experience 입력과 함께 처리하는 게 낫다.
3. **`labels`·`hidden-gluten`에는 용어 로마자가 남아 있다**(원재료명=wonjaeryomyeong 식). 따라 말하기용이 아니라 영어권 독자의 식별자라 남겼다 — **"발음 기호는 전부"가 운영자 의도면 그 2편도 걷어낼 것**(재채점 2라운드).
4. FAQ 1이 rich result 렌더 길이를 넘겨 간장 단서가 잘린다.
