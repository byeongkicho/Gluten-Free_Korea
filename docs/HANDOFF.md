...
- **마지막 업데이트:** 2026-08-20 10:31
- **작업자:** Claude Code
- **마지막 커밋:** `4a1323a` seo(adsense): deindex thin place pages, add Terms and Contact
- **브랜치:** main
...
- **🔧 post-commit 훅 앵커 1개 결번이었음 (08-20 수정)** — `scripts/hooks/post-commit.sh`는 세 필드(`마지막 업데이트`·`브랜치`·`마지막 커밋`)를 갱신하는데 **`- **마지막 커밋:**` 라인이 문서에 없어** 매 커밋 stderr 경고만 내고 있었다. 08-07에 "조용히 죽지 말고 알리게" 고친 설계는 제대로 작동했고, **알림을 아무도 읽지 않은 것**이 문제였다. 라인 신설로 해소.
